# MITRE ATLAS: Complete Technique Catalog

Authoritative list of valid ATLAS technique IDs and names (adversarial threats to AI/ML and LLM systems). Use ONLY these IDs when mapping threats against an LLM element in the DFD. Sub-techniques are indented under their parent; cite the sub-technique ID when it is the precise match.

_Source: MITRE ATLAS (atlas.mitre.org). 101 techniques, 69 sub-techniques. Catalog snapshot vendored 2026-09. Echo this line in the report appendix and re-verify any cited ID against the live matrix, which MITRE revises periodically._
